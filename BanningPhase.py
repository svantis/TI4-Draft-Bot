...
# check if the draft has not progressed to the nomination phase
with open(f"./Drafts/{file_name}", "r") as f:
    draft_text = f.read()
    if "Nomination Phase" in draft_text:
        print("The draft has progressed to the Nomination Phase. Banning is not allowed anymore.")
        exit()
...
# get the factions the player already picked in the picking phase
with open(f"./Drafts/{file_name}", "r") as f:
    draft_text = f.read()
    picked_factions = []
    for line in draft_text.split("\n"):
        if f"Faction added by player {player_name}: " in line:
            picked_factions.append(line.split(": ")[1])

# get the valid factions the player can pick
with open(f"./Drafts/{file_name}", "r+") as f:
    draft_text = f.read()
    valid_factions = [f.strip("[]\"' ").replace("'", "").replace("\"", "") for f in draft_text.split(f"Factions for player {player_name}: ")[1].split("\n")[0].split(",")]
    valid_factions = [f for f in valid_factions if f not in picked_factions]
    while True:
        chosen_faction = input(f"Remaining Faction Pool ({', '.join(valid_factions)}): ")
...
# write the result
    ...
    for i in range(len(draft_lines)):
        if f"Faction banned by player {player_name}: " in draft_lines[i]:
